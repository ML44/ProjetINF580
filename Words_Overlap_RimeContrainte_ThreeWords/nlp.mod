...
var mot3{N} binary, default 0;

#variables pour linéariser la fonction objectif
var M12{N, N} binary, default 0;#mot1[i]*mot2[j] = M12[i, j]
var M23{N, N} binary, default 0;#mot2[i]*mot3[j] = M23[i, j]

#ne pas dépasser le nombre de syllabes permis
subject to NotTooMuchSyllables : sum{i in N} syllabes[i] * (mot1[i] + mot2[i] + mot3[i]) <= LongueurMax;
...
subject to EmptyWordUse1_3 : (mot3[0] * LongueurMax) - (sum{i in N} syllabes[i] * M23[i, 0]) = 0;
subject to EmptyWordUse2_3 : mot3[0] + (LongueurMax - sum{i in N} syllabes[i] * (mot1[i] + mot2[i])) >= 1;
...
subject to No2ZeroSyllablesWords3 : sum{i in N} mot3[i]*syllabes[i] + sum{i in N} mot2[i]*syllabes[i] >= 1 - mot3[0];

#ne sélectionner qu'un seul mot
subject to OnlyOneWord1 : sum{i in N} mot1[i] = 1;
subject to OnlyOneWord2 : sum{i in N} mot2[i] = 1;
subject to OnlyOneWord3 : sum{i in N} mot3[i] = 1;
...
subject to linearization1_23{i in N, j in N} : M23[i, j] <= mot2[i];
subject to linearization2_23{i in N, j in N} : M23[i, j] <= mot3[j];
subject to linearization3_23{i in N, j in N} : M23[i, j] >= mot2[i] + mot3[j] - 1;

maximize objective : (sum{i in N} enchainement[i, lastWord]*mot1[i]) + sum{i in N, j in N} enchainement[i, j]*(M12[i, j] + M23[i, j]);